...
WebpackAssetsPlugin.prototype.apply = function (compiler) {
  compiler.plugin('emit', (compilation, cb) => {
    ...
    const asyncChunks = {}
    stats.chunks.filter(c => !c.entry && !c.initial)
      .map(chunk => {
        const {id, size, files, hash, modules, origins} = chunk
        const file = files[0]
        const name = modules[0].name
        const src = name.split(' ')[0]
        const chunkName = origins[0].name
        asyncChunks[chunkName || id] = {id, size, chunkName, hash, src, file}
      })
